...
### RTL Simulaiton
<pattern 1>
1. テストベンチの追加:Vivadoで「Add Sources」→「Add or create simulation sources」を選択し、tb_SNN_wrapper_csv.v を追加する。（※絶対に Design Sources には入れないこと）。 (他の.vファイルも必要。テストベンチだけでは動かない.ex...```SNN_wrapper.v```, etc...)
...
あなたが作成した完璧なテストベンチ `tb_SNN_wrapper_csv.v` を登録する。  
 (他の.vファイルも必要。テストベンチだけでは動かない.ex...```SNN_wrapper.v```, etc...)  
